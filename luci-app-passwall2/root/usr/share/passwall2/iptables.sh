				...
				[ -n "$shunt_node" ] && {
					...
					[ "${enable_geoview}" = "1" ] && {
						...
						[ -n "$_geoip_code" ] && {
							...
							echolog "  - [$?]Analyze the shunt rules[$shunt_id]-[geoip:${_geoip_code}]Join IPSET Finish"
						}
					}
				}
			done
		...
		for sid in $(ls -F ${TMP_ACL_PATH} | grep '/$' | awk -F '/' '{print $1}' | grep -v 'default'); do
			...
			[ "${write_ipset_direct}" = "1" ] && {
				if [ -n "$(get_cache_var "ACL_${sid}_default")" ]; then
					...
					#Diversion rulesIPList(Import when using shunt node)
			...
			for i in $(cat $_acl_list); do
				...
				[ "$tcp_no_redir_ports" != "disable" ] && {
					if [ "$tcp_no_redir_ports" != "1:65535" ]; then
						...
						echolog "  - ${msg}No agent TCP port[${tcp_no_redir_ports}]"
					else
						#Will be at the endreturn，No need to add extra rules。
						tcp_proxy_mode="disable"
						echolog "  - ${msg}Not all agent TCP"
					fi
				}
				...
				[ "$udp_no_redir_ports" != "disable" ] && {
					if [ "$udp_no_redir_ports" != "1:65535" ]; then
						...
						echolog "  - ${msg}No agent UDP port[${udp_no_redir_ports}]"
					else
						#Will be at the endreturn，No need to add extra rules。
						udp_proxy_mode="disable"
						echolog "  - ${msg}Not all agent UDP"
					fi
				}
				...
				if ([ "$tcp_proxy_mode" != "disable" ] || [ "$udp_proxy_mode" != "disable" ]) && [ -n "$dns_redirect_port" ]; then
					...
					[ -z "$(get_cache_var "ACL_${sid}_default")" ] && echolog "  - ${msg}Use different nodes than global configuration，AlreadyDNSForce redirect to dedicated DNS server。"
				else
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && [ "$CLIENT_PROXY" == 1 ] && {
		...
		[ "$TCP_NO_REDIR_PORTS" != "disable" ] && {
			...
			if [ "$TCP_NO_REDIR_PORTS" != "1:65535" ]; then
				...
				TCP_PROXY_MODE="disable"
				echolog "  - ${msg}Not all agent TCP port"
			fi
		}
		...
		[ "$UDP_NO_REDIR_PORTS" != "disable" ] && {
			...
			if [ "$UDP_NO_REDIR_PORTS" != "1:65535" ]; then
				...
				UDP_PROXY_MODE="disable"
				echolog "  - ${msg}Not all agent UDP port"
			fi
		}

		if ([ "$TCP_PROXY_MODE" != "disable" ] || [ "$UDP_PROXY_MODE" != "disable" ]) && [ -n "$NODE" ]; then
			[ -n "$DNS_REDIRECT_PORT" ] && {
				$ipt_n -A PSW2_DNS $(comment "default") -p udp --dport 53 -j REDIRECT --to-ports $DNS_REDIRECT_PORT
				$ip6t_n -A PSW2_DNS $(comment "default") -p udp --dport 53 -j REDIRECT --to-ports $DNS_REDIRECT_PORT 2>/dev/null
				$ipt_n -A PSW2_DNS $(comment "default") -p tcp --dport 53 -j REDIRECT --to-ports $DNS_REDIRECT_PORT
				$ip6t_n -A PSW2_DNS $(comment "default") -p tcp --dport 53 -j REDIRECT --to-ports $DNS_REDIRECT_PORT 2>/dev/null
			}
		fi

		if [ "$TCP_PROXY_MODE" != "disable" ] && [ -n "$NODE" ]; then
			msg2="${msg}use TCP node[$(config_n_get $NODE remarks)]"
			if [ -n "${is_tproxy}" ]; then
				msg2="${msg2}(TPROXY:${REDIR_PORT})"
				ipt_j="-j PSW2_RULE"
			else
			...
			[ "$accept_icmp" = "1" ] && {
				...
				add_shunt_t_rule "${SHUNT_LIST4}" "$ipt_n -A PSW2 $(comment "default") -p icmp" "$(REDIRECT)"
				$ipt_n -A PSW2 $(comment "default") -p icmp $(REDIRECT)
			}
			
			[ "$accept_icmpv6" = "1" ] && [ "$PROXY_IPV6" == "1" ] && {
				$ip6t_n -A PSW2 $(comment "default") -p ipv6-icmp -d $FAKE_IP_6 $(REDIRECT)
				add_shunt_t_rule "${SHUNT_LIST6}" "$ip6t_n -A PSW2 $(comment "default") -p ipv6-icmp" "$(REDIRECT)"
				$ip6t_n -A PSW2 $(comment "default") -p ipv6-icmp $(REDIRECT)
			}

			$ipt_tmp -A PSW2 $(comment "default") -p tcp -d $FAKE_IP ${ipt_j}
			add_shunt_t_rule "${SHUNT_LIST4}" "$ipt_tmp -A PSW2 $(comment "default") -p tcp $(factor $TCP_REDIR_PORTS "-m multiport --dport")" "${ipt_j}"
			$ipt_tmp -A PSW2 $(comment "default") -p tcp $(factor $TCP_REDIR_PORTS "-m multiport --dport") ${ipt_j}
			[ -n "${is_tproxy}" ] && $ipt_m -A PSW2 $(comment "default") -p tcp $(REDIRECT $REDIR_PORT TPROXY)

			[ "$PROXY_IPV6" == "1" ] && {
				$ip6t_m -A PSW2 $(comment "default") -p tcp -d $FAKE_IP_6 -j PSW2_RULE
				add_shunt_t_rule "${SHUNT_LIST6}" "$ip6t_m -A PSW2 $(comment "default") -p tcp $(factor $TCP_REDIR_PORTS "-m multiport --dport")" "-j PSW2_RULE"
		...
		if [ "$UDP_PROXY_MODE" != "disable" ] && [ -n "$NODE" ]; then
			...
			add_shunt_t_rule "${SHUNT_LIST4}" "$ipt_m -A PSW2 $(comment "default") -p udp $(factor $UDP_REDIR_PORTS "-m multiport --dport")" "-j PSW2_RULE"
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				add_shunt_t_rule "${SHUNT_LIST6}" "$ip6t_m -A PSW2 $(comment "default") -p udp $(factor $UDP_REDIR_PORTS "-m multiport --dport")" "-j PSW2_RULE"
...
filter_haproxy() {
	...
	echolog "Join load balancing nodes toipset[$IPSET_VPS]Direct connection is completed"
}
...
filter_vpsip() {
	...
	echolog "  - [$?]Join allIPv4Node toipset[$IPSET_VPS]Direct connection is completed"
	...
	echolog "  - [$?]Join allIPv6Node toipset[$IPSET_VPS6]Direct connection is completed"
}
...
add_firewall_rule() {
	...
	[ -n "$lan_ifname" ] && {
		...
		#echolog "This machineIPv4Direct network access to network：${lan_ip}"
		#echolog "This machineIPv6Direct network access to network：${lan_ip6}"
	...
	[ -n "$ISP_DNS" ] && {
		...
		for ispip in $ISP_DNS; do
			...
			echolog "  - [$?]AdditionalISP IPv4 DNSGo to the whitelist：${ispip}"
		done
	}
	...
	[ -n "$ISP_DNS6" ] && {
		...
		for ispip6 in $ISP_DNS6; do
			...
			echolog "  - [$?]AdditionalISP IPv6 DNSGo to the whitelist：${ispip6}"
		done
	}
	...
	[ -n "$AUTO_DNS" ] && {
		for auto_dns in $(echo $AUTO_DNS | tr ',' ' '); do
			...
			echolog "  - [$?]Additional direct connectionDNSarriveiptables：${dns_address}:${dns_port:-53}"
		done
	}
	...
	[ "$ENABLED_DEFAULT_ACL" == 1 ] && {
		...
		if [ -n "$NODE" ] && [ "$TCP_LOCALHOST_PROXY" = "1" ]; then
			...
			[ -n "${is_tproxy}" ] && {
				$ipt_m -A PSW2 $(comment "This machine") -p tcp -i lo $(REDIRECT $REDIR_PORT TPROXY)
				$ipt_m -A PSW2 $(comment "This machine") -p tcp -i lo -j RETURN
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				$ip6t_m -A PSW2 $(comment "This machine") -p tcp -i lo $(REDIRECT $REDIR_PORT TPROXY)
				$ip6t_m -A PSW2 $(comment "This machine") -p tcp -i lo -j RETURN
		...
		if [ -n "$NODE" ] && [ "$UDP_LOCALHOST_PROXY" = "1" ]; then
			...
			$ipt_m -A PSW2 $(comment "This machine") -p udp -i lo $(REDIRECT $REDIR_PORT TPROXY)
			$ipt_m -A PSW2 $(comment "This machine") -p udp -i lo -j RETURN
			...
			[ "$PROXY_IPV6" == "1" ] && {
				...
				$ip6t_m -A PSW2 $(comment "This machine") -p udp -i lo $(REDIRECT $REDIR_PORT TPROXY)
				$ip6t_m -A PSW2 $(comment "This machine") -p udp -i lo -j RETURN
	...
	echolog "Firewall rules are loading！"
}
...
del_firewall_rule() {
	...
	$DIR/app.sh echolog "delete iptables Rules completed。"
}

flush_ipset() {
	$DIR/app.sh echolog "Clear IPSet。"
